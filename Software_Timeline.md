...
- [x] Extended the COPPA/FERPA memo (`docs/coppa_ferpa_determination.md`) to cover SOPIPA + state student-privacy laws — done (Jul 4), now that the counselor dashboard has actually shipped and the memo's own "revisit before Phase 2 rollout" trigger condition is live. Informal, non-lawyer read, same as the rest of the doc: SOPIPA-style laws are triggered by a formal operator-school/district relationship, which doesn't exist yet (counselor accounts are still self-serve, no district contract or self-serve school signup flow built). Flags exactly when that flips and needs a real legal read before it happens.
- [x] Real WCAG 2.1 AA accessibility pass — done (Jul 4), well beyond the earlier partial pass (focus states/keyboard nav/aria labels from Fast-follow, below). Computed actual contrast ratios for every color token: found `--secondary` (#737373) failed 4.5:1 against `--bg`/`--card`/its own pill background (3.3–4.2:1) — fixed by shifting to #8a8a8a, which passes ≥4.5:1 everywhere it's used while staying the dimmest of the five semantic tones (design intent preserved). Found and fixed that **no form label anywhere in the app was programmatically associated with its input** (no `htmlFor`/`id` pairing on login, signup, onboarding, or profile-edit forms) — screen readers couldn't announce field names; fixed across all four. Added `aria-label` to unlabeled inputs/selects (search, filters, add-school, add-timeline-item, essay draft, counselor notes, send-reminder). Added `role="alert"`/`role="status"` to every error and status message across the app (previously silent to screen readers) and `aria-pressed`/`role="group"` to the campus-size/setting toggle-button groups. Added a skip-to-main-content link + `aria-current="page"` on nav links, both previously absent. **Genuinely open:** the border color (`--border` #2A2A2A) fails WCAG's 3:1 non-text contrast requirement for UI component boundaries — flagged, not changed, since it's a CLAUDE.md-locked design-system value used on every card/input/button and changing it site-wide is a bigger visual design call than a targeted fix; full AA "certification" would still need real screen-reader/assistive-tech testing, which no amount of code auditing substitutes for.
...
### 7. Thorough desktop/laptop testing — DONE (Jul 4/5)
- [x] Full end-to-end walkthrough of the entire student flow in a desktop browser (Chromium via dev preview): sign up → onboarding → matches generate → regenerate → school detail → match breakdown → timeline generate → "you are here" marker → task detail/complete → essay feedback → profile edit/save → account deletion. Every step exercised for real against live Supabase/Anthropic — **found and fixed two real bugs**: (1) `/api/matches/generate` had no retry and could 502 on a single malformed/truncated model response — switched to forced tool-use plus one bounded retry with token headroom (`src/app/api/matches/generate/route.ts`); (2) the dashboard's "Generate Timeline" card didn't stretch to match the matches card's height when matches had more content than the empty-timeline prompt — fixed the grid-stretch chain (`src/app/dashboard/page.tsx`, `src/app/dashboard/GenerateTimelineCard.tsx`).
- [x] Check both Chrome and Safari on the laptop (Safari catches issues Chrome hides) — Chrome-equivalent (Chromium preview) walkthrough done as above; Safari pass done separately by Ari, confirmed working end-to-end aside from the dashboard card-height bug above (now fixed).
...
### 8. Mobile device QA + troubleshooting — DONE (Jul 4/5)
- [x] Walk the full student flow on a real phone against the dev server (tunneled via cloudflared for a real HTTPS origin, since Google/Microsoft OAuth reject plain-HTTP LAN IPs): every screen renders correctly, no blank sections, text/buttons/layout all present and tappable, no broken hero/WebGL content. **Found and fixed three real bugs surfaced only by real on-device testing:**
  1. Dashboard's "top matches" and "Generate Timeline" cards mismatched height when one had more content than the other — grid-stretch chain wasn't propagating through an extra wrapper div (`src/app/dashboard/page.tsx`, `src/app/dashboard/GenerateTimelineCard.tsx`).
  2. Google/Microsoft "Continue with..." buttons did nothing — `signInWithOAuth` resolves with an authorize URL but doesn't navigate the browser itself in this SDK/config; the code only checked for `error` and never used `data.url`. Fixed in both `src/app/login/page.tsx` and `src/app/signup/page.tsx`.
