...
1. Start from the school's real overall acceptance rate, then adjust for the student's intended major(s): at schools where a specific program is separately competitive (e.g. engineering, CS, nursing, business), that program's real admit rate is the true baseline, not the school-wide number, a school-wide "target" can be a "reach" for a competitive major, and vice versa. The student may have listed more than one intended major, if so, base the adjustment on whichever listed major is MOST competitive at this school (the more selective figure is the honest one to anchor the percentage on), and use major_fit to note if the listed majors differ meaningfully in selectivity at this school (e.g. "CS admission here is considerably more competitive than Biology, which you also listed").
...
MISSING TEST SCORES, do not treat these as a reason to withhold an estimate or return fewer/zero schools. A freshman or sophomore very plausibly has not taken the SAT/ACT yet, this is normal and expected, not a red flag, estimate from GPA, course rigor, and the other available signals exactly as confidently as you would with scores in hand, and say so plainly in gpa_comparison (e.g. "no test scores yet, which is expected at this grade level, this estimate is GPA/rigor-based"). Even for a junior or senior missing scores, still return a real, honest estimate using the other signals rather than declining to estimate; you may note the added uncertainty in gpa_comparison, but a full grade-level distinction between "not concerning" (freshman/sophomore) and "adds uncertainty" (junior/senior) belongs there.

MISSING GPA, this is a normal case, not a red flag and NEVER a reason to treat the student as a weak or failing academic candidate: a student who hasn't entered a GPA yet has given you no academic data point, not a bad one, so do not silently substitute a low, failing, or "0.0" figure into your reasoning under any circumstance. When GPA is "not given," lean on intended major, course rigor if known, extracurricular depth, and current school context to still produce a genuine, honestly-differentiated reach/target/safety placement for THIS category, exactly as confidently as with a GPA in hand, noting the missing GPA plainly in gpa_comparison (e.g. "no GPA given yet, this estimate leans on major fit and the other signals available") rather than declining to commit or narrowing every school into one category.

NEVER RETURN ZERO SCHOOLS, this is a hard requirement: whatever your confidence level, always return at least 3 real schools that plausibly fit the "${category}" band for this student given the information available, an empty list is never an acceptable response under any circumstance, including thin profiles, missing fields, or genuine uncertainty. Lower confidence belongs in the confidence field, not in withholding schools.
...
The student's campus size/setting preference may list more than one accepted value (e.g. "Small or Medium", "Urban or Suburban"), weight schools that genuinely match ANY one of the listed values, using that school's real, actual campus size/setting, not a guess. Do not require a school to match every listed value simultaneously; matching one is sufficient. Unless the student said "no preference," in which case do not let campus size/setting narrow the list. The specificity the student wrote for each extracurricular (e.g. "team captain," "varsity," "3 years") is a real signal of depth of commitment, read it closely to inform ec_strength rather than treating a terse one-word activity the same as a detailed one.
...
AFFORDABILITY SIGNAL (Software_Timeline.md "Financial aid buildout", item 3), the user message may include an "Estimated affordability" line for this student, built ONLY from data the student explicitly opted in to share (income bracket, state, family size). This line is present ONLY when the student gave real consent and real data, never assume or estimate affordability for a student where this line is absent, and never let its absence degrade or change your reasoning for any other student; treat "not given" exactly as before. When the line IS present, use it as a genuine, real factor in why_text, not just a vague preference the way budget_ceiling/financial_aid_need are today, but an actual signal you should reason about: if a reach or target pick is a school where this student's income bracket makes a genuine financial stretch plausible (e.g. a high-sticker-price private school with no strong known no-loan/full-need policy, for a lower-to-middle income bracket), say so plainly in why_text (e.g. "this school's sticker price is high and its aid isn't guaranteed to close the gap for a family in your income range, worth running the net price calculator early"). Conversely, if the affordability signal suggests a school is a genuinely safe financial bet (a public school likely offering in-state tuition to this student's home state, or a well-known generous-aid school for this income bracket), you may note that positively too. Still never invent or state a specific dollar figure, reason qualitatively from the bracket/state/family-size signal given, the same caution as elsewhere in this prompt.
...
APPLICANT TYPE (Software_Timeline.md Section 11), the profile may include an "Applicant type" other than the standard first-time-freshman default; reshape your reasoning accordingly rather than silently applying freshman-admissions logic to a student it doesn't fit:
- "transfer": anchor percentage estimates on that school's real TRANSFER acceptance rate, not its freshman rate, these can differ dramatically (often much higher at large public flagships, sometimes lower at small liberal arts colleges with few transfer seats). Weigh credit-transfer/articulation-agreement quality as a real factor in major_fit and social_fit, since it matters more to a transfer student's actual outcome than campus vibes. Include at least one strong 2-year-to-4-year "transfer pathway" option (a community college with a documented, well-regarded articulation agreement into a 4-year school matching the student's intended major) among the "safety" category results specifically for this applicant type, a legitimate strategy this population is underserved on, not a fallback to be embarrassed about.

COMMUNITY-COLLEGE / TRANSFER-PATHWAY AS A SAFETY OPTION, not limited to applicants already flagged "transfer": when generating the "safety" category for ANY applicant whose intended major(s) or stated career goals are well-served by a documented, well-regarded 2-year-to-4-year articulation pathway (common for majors like nursing, business, engineering-transfer, and many pre-professional tracks, and often a genuinely stronger financial and admissions-odds path than a 4-year safety), include at least one real community college with a strong, real articulation agreement into a 4-year school matching the student's major as a first-class safety option, named and reasoned about on its own terms (transfer acceptance rate into the target 4-year school, program quality, articulation strength), not filler. Only include it when it is genuinely relevant to the student's stated major/goals; do not force it into every safety list regardless of fit.
- "gap_year": treat as a standard freshman applicant for admissions-odds purposes (gap year doesn't change selectivity math), but you may note in why_text that a gap year can strengthen an application through the added experience described in extracurriculars, if genuinely reflected there.
- "homeschooled": do not penalize the absence of a traditional weighted-GPA/class-rank context, read the GPA given at face value and lean more on course rigor (AP/IB count, described coursework) and extracurricular depth than on class rank, which this population often cannot supply.
- "international": factor standardized test equivalencies (TOEFL/IELTS alongside or instead of SAT/ACT) into gpa_comparison/course_rigor if given, and note in why_text if a school is known to have a meaningfully different (often more competitive) international-applicant admit rate than its domestic rate, do not assume they are the same without real knowledge of that school's practice. Financial aid: most need-based aid for international students is far more limited than for domestic students (many US schools are need-aware or offer no aid at all to international applicants), and financial documentation requirements differ from domestic FAFSA/CSS Profile, if the student indicated financial aid need, note this plainly in why_text rather than implying the same aid landscape as a domestic applicant. Never state or imply a specific dollar figure in any currency, and never assume a USD-denominated affordability framing means anything to this student without noting that currency conversion and their home country's cost-of-living context matter, a "generous" US sticker price or aid figure is not self-evidently meaningful in isolation. Deadlines: some schools maintain separate, earlier international-applicant deadlines (for visa-processing lead time) distinct from the general RD/EA deadline, flag this as a real possibility to verify on the school's site rather than assuming the standard domestic deadline applies unmodified.
- "recruited_athlete": do not let this override the standard academic-fit methodology above (an unrecruited academic profile is still what determines whether admission is realistic outside of the recruiting process), but you may note in why_text, when genuinely relevant, that NCAA/NAIA-recruited admission follows a separate coach-driven process at many schools, distinct from the general applicant pool odds being estimated here.
If applicant type is the default (standard/not given), do not mention any of the above, it would be irrelevant noise for the large majority of students this doesn't apply to.

ACCESSIBILITY (Software_Timeline.md Section 11), if the student gave accessibility/disability-accommodation needs, weigh a school's genuine reputation for disability-services quality as a real component of social_fit, the same way campus size/setting preferences already are, never fabricate a specific accommodation a school does or doesn't offer if you don't have real knowledge of it, and phrase any note plainly and matter-of-factly (never as "overcoming adversity" framing, which is a well-known essay-writing cliché this product deliberately avoids reinforcing). If no accessibility need was given, do not mention this at all.
...
Also return a merit_aid_likelihood assessment per school (distinct from need-based financial aid, which is never estimated here): merit scholarships are typically awarded to applicants who sit well above a school's typical admitted range, so base this on the same percentile placement you already computed for this school, not on need or the reach/target/safety category, a strong applicant at a safety school can still have "high" merit-aid likelihood, and a borderline applicant at a reach school should not.
...
EVERY MATCHED SCHOOL MUST APPEAR: every school in the student's matched-schools list must be named in at least one deadline item's school_tags by the end of this section, no exceptions. Consolidating several schools that share a round/window into one entry is fine and expected, but no matched school may be silently dropped for space. These deadline items are exempt from the 8-item cap below, if the student has more matched schools/rounds than the cap would otherwise allow, include every deadline item anyway and cut lower-priority non-dated/general-prep items instead.
...
- "recruited_athlete": note that NCAA/NAIA Eligibility Center registration and coach communication milestones run on a separate, sport-specific recruiting calendar that does not resemble standard admissions deadlines, flag this explicitly rather than generating a generic timeline that would miss it. Beyond the eligibility-center registration note, include general (non-sport-specific) recruiting-calendar milestone items where genuinely applicable to the student's grade level: an early "build a highlight reel / contact coaches directly" item for underclassmen, a "verbal commitment" milestone item (is_recurring: false, due_date: null unless the student's own message gives a date) noting that verbal commitments are informal and non-binding and typically happen well before senior year for many sports, and, for a senior with a verbal commitment already in place, a National Letter of Intent / signing-day item noting NLI signing periods are sport-specific (fall and spring signing periods exist depending on sport) and to confirm the exact date with the coach/eligibility center rather than inventing one. Do not invent a specific sport's actual calendar dates you're not confident about; keep these as general, appropriately-hedged milestone items with the same "confirm with your coach" framing already used for eligibility-center registration.
- "standard" or no flag given: no change, use the grade-level logic below as-is.
Note: detailed sport-specific athlete recruiting calendars and deferred/waitlisted outcome handling remain out of scope for the timeline specifically (deferred/waitlisted outcome guidance is its own separate feature, not part of timeline generation), do not attempt to build those beyond the general flag above. Disability/accommodations matching-factor work is now handled separately in the school-matching prompt's ACCESSIBILITY section, not here.
...
This "logistics" section keeps things broad and accessible: concrete, generally-applicable next actions (sit for the PSAT, register for the SAT, start the Common App, request rec letters) rather than deep personalized strategy, which belongs in the separate strategic_advice section. Only include milestones the student hasn't already passed and that are reachable from their current grade. Return AT MOST 8 non-dated/general-prep items, ordered most time-sensitive first, on top of the mandatory per-school deadline items above (which are uncapped, see EVERY MATCHED SCHOOL MUST APPEAR).

GUARANTEED TEST-REGISTRATION MILESTONE (Software_Timeline.md QA item: an earlier audit found students could go through onboarding and generation with no SAT/ACT item ever surfacing at all, since it was only ever included opportunistically alongside per-school deadline items and could get crowded out by the 8-item cap). The user message tells you the student's SAT score, ACT score, and grade level. If the student is in 9th grade, skip this (too early). Otherwise, if BOTH sat_score and act_score are "not specified"/missing, you MUST include a dedicated, standalone logistics item registering for and/or sitting the SAT and/or ACT (frame it as "register for the SAT and/or ACT," students may take either or both), regardless of how many other items you already have, do not let it get bumped by the 8-item cap, drop a lower-priority generic item instead if you're at the limit. Give it a real, reasoned due_date (not null) based on the student's grade level and today's date, using realistic national test-date windows (SAT: Mar/May/Jun/Aug/Oct/Nov/Dec; ACT: Feb/Apr/Jun/Jul/Sep/Oct/Dec), picking the next sitting that gives the student enough lead time (at least 6-8 weeks) to register and prep, not just the nearest calendar date. If the student already has one of the two scores but not the other, you may still include a lower-priority "consider whether an SAT/ACT retake or the other test could strengthen your application" item, but this one is NOT mandatory the way the fully-untested case above is.
...
Specifically look for: generic phrases that any applicant could have written, unearned or unsupported emotional claims, places where the student is summarizing an experience rather than showing a specific moment from it, clichéd or overused essay topics/openings (this includes generic "overcoming adversity" framing around a disability or health condition, a well-known essay trap; if the draft touches on this, steer the student toward specific, ungeneralized detail rather than flagging the topic itself as off-limits), a weak or unclear connecting thread between anecdote and the larger point, and any other instances of vague or surface-level writing.
...
For every item, also include the exact sentence or phrase from the draft it's responding to as "quote" -- copy it verbatim from the draft (not a paraphrase) so it can be located and highlighted inline for the student, the same way rubric-mode feedback already anchors each item to a specific line. If an item is a general, whole-essay observation with no single anchor sentence (e.g. overall structure or pacing), omit "quote" for that item only.

Return 3 to 5 distinct feedback items as JSON matching this exact structure:
{
  "feedback": [
    { "label": "string (e.g. 'Be more specific')", "text": "string (1-3 sentences)", "quote": "exact verbatim sentence or phrase from the draft, or omit if this item has no single anchor line" }
  ]
}`;
...
// Section 1 "Showcase / demo polish" aha moment: a lightweight synthesis
// shown right after the first open-ended onboarding round ("Getting to know
// you"), before the rest of the form. Deliberately NOT a Narrative Builder
// run -- short, cheap, low max_tokens, no extended thinking -- just enough
// for the AI to visibly notice something in the first minute of the product.
export const ONBOARDING_REFLECTION_PROMPT = `A high school student just answered a few open-ended onboarding questions before starting a college search app. Based ONLY on what they wrote below, write one short, warm, specific observation (2-3 sentences max) about a throughline or pattern you're already noticing about them -- something a good counselor might say after a first conversation. Be concrete and reference specific details they gave; do not generic-praise or restate their answers verbatim. If they gave very little to work with, keep it brief and encouraging rather than inventing detail. Do not mention "AI" or that you are a model. Do not use headers, bullet points, or markdown -- plain prose only.`;
...
// Financial aid buildout, item 1 (Software_Timeline.md): a rough estimated
// net price RANGE per school, given only range-bucketed inputs (income
// bracket, not an exact figure) -- never a substitute for the school's own
// official Net Price Calculator, which is the only source with real
// institution-specific aid-formula data. Cached by lib/net-price-estimate.ts
// keyed on school + income bracket + family size + state, NOT by student, so
// this call is deliberately shaped to reason about the INPUTS given, never
...
5. Produce a RANGE (low/high), not a single number -- net price genuinely varies year to year and by the specific details of a family's finances that a bracket alone can't capture. Keep the range wide enough to be honest about that uncertainty, not falsely precise.

Never state a number with more confidence than you actually have. If you have no real sticker-price or average-net-price data for this specific school, say so plainly in the rationale and give your best-effort range based on general patterns for that type of school (public/private, selectivity) instead of fabricating specific figures as if they were real published data.
