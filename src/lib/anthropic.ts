...
If the student listed schools they're already considering, evaluate EACH one using the same methodology above, and INCLUDE it in this category's 5 if it genuinely belongs in the "${category}" band for this student — do not leave out a school just because the student already named it; that is the opposite of what they want. Only exclude a named school from this category if it clearly belongs in a different category instead (e.g., it's really more of a target than a reach for this student). After placing whichever of the student's named schools genuinely fit "${category}", fill any remaining slots with additional real schools that match the taste signal in their list (selectivity level, size, region, etc.) — the goal is a list that still has variety, not one padded entirely with schools they didn't name, and not one that omits the schools they told you they care about. Favor a mix of school types (size, region, public/private) among the additions rather than 5 near-identical schools, unless the student's profile clearly points to a narrow type they want. For "safety" specifically: only include schools the student would plausibly be glad to attend given their stated preferences and considering-list, not schools that are safe purely on paper.
