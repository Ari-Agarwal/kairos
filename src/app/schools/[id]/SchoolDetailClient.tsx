...
export default function SchoolDetailClient({
  ...
  async function answerAidNudge(value: boolean) {
    setAidNeed(value);
    const supabase = createClient();
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;
    await supabase.from("profiles").update({ financial_aid_need: value }).eq("user_id", user.id);
  }
  ...
  return (
    <div className="px-5 md:px-8 py-8 max-w-2xl mx-auto w-full">
      ...
      <Link
        href={`/career-path?school=${encodeURIComponent(match.school_name)}`}
        className="inline-flex items-center gap-1.5 text-primary hover:text-primary-hover text-sm mb-6 underline underline-offset-2"
      >
        See Career Path for {match.school_name} →
      </Link>
                ...
                {aidNeed === null && !aidNudgeDismissed && (
                  <div className="bg-bg border border-border rounded-xl p-3 mb-3 flex items-center justify-between gap-3 flex-wrap">
                    ...
                    <div className="flex gap-2 shrink-0">
                      <button
                        onClick={() => answerAidNudge(true)}
                        className="rounded-lg bg-primary hover:bg-primary-hover transition-colors text-bg text-sm font-medium px-3 py-1.5"
                      >
                        Yes
                      </button>
                      <button
                        onClick={() => answerAidNudge(false)}
                        className="rounded-lg border border-border text-text-gray hover:text-text text-sm px-3 py-1.5 transition-colors"
                      >
                        No
                      </button>
                      <button
                        onClick={() => setAidNudgeDismissed(true)}
                        className="text-text-gray hover:text-text text-sm px-2 py-1.5"
                        aria-label="Dismiss"
                      >
                        Skip
                      </button>
